...
class RandomUserAgentMiddlerware(object):
    ...
    def process_request(self, request, spider):  # 系统电泳函数
        def get_ua():  # 主要是用来让用户选择获取什么样的UserAgent,火狐，谷歌，IE都可以
            return getattr(self.ua, self.ua_type)
        request.headers.setdefault(b'User-Agent', get_ua())


class RandomCompanyProxyMiddlerware(object):
...
class MyRtryMiddlerware(RetryMiddleware):
    """
    重写scrapy自带的重试中间件，达到最大重试次数还是不成功的链接保存到数据库
    """
    """
        重新定义重试中间件
        """
